let handler = async (m, { conn, args, participants, isAdmin, isBotAdmin, isOwner, isPrems, usedPrefix, command }) => {
  ...
  try {
    ...
    let txt = `╭─「 ✦ 𓆩💪𓆪 ᴛᴏᴘ ᴍᴀᴄʜᴏs ᴅᴇʟ ɢʀᴜᴘᴏ ✦ 」─╮\n`
    txt += `│\n`
    ...
    console.error('Error en top machos:', e)
    return conn.sendMessage(m.chat, {
      text: '《✧》Ocurrió un error al generar el top de machos del grupo.',
      contextInfo: {
        ...rcanal.contextInfo
      }
    }, { quoted: m })
...
handler.help = ['#topmachos']
handler.tags = ['fun', 'grupos']
handler.command = ['topmachos', 'topmacho', 'machos', 'machotop']
handler.group = true
