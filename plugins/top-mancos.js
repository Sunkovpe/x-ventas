let handler = async (m, { conn, args, participants, isAdmin, isBotAdmin, isOwner, isPrems, usedPrefix, command }) => {
  ...
  try {
    ...
    let txt = `╭─「 ✦ 𓆩🤡𓆪 ᴛᴏᴘ ᴍᴀɴᴄᴏs ᴅᴇʟ ɢʀᴜᴘᴏ ✦ 」─╮\n`
    txt += `│\n`
    ...
    console.error('Error en top mancos:', e)
    return conn.sendMessage(m.chat, {
      text: '《✧》Ocurrió un error al generar el top de mancos del grupo.',
      contextInfo: {
        ...rcanal.contextInfo
      }
    }, { quoted: m })
...
handler.help = ['#topmancos']
handler.tags = ['fun', 'grupos']
handler.command = ['topmancos', 'topmanco', 'mancos', 'mancotop']
handler.group = true
