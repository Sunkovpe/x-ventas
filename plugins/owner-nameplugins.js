...
${usedPrefix}nameplugins test.js > nuevo-test.js`)
  ...
  try {
    ...
    let txt = `╭─「 ✦ 𓆩🔄𓆪 ᴘʟᴜɢɪɴ ʀᴇɴᴏᴍʙʀᴀᴅᴏ ✦ 」─╮\n`
    txt += `│\n`
    txt += `╰➺ ✧ *Archivo original:* ${oldFileName}\n`
    txt += `╰➺ ✧ *Nuevo nombre:* ${newFileName}\n`
    txt += `╰➺ ✧ *Estado:* Renombrado exitosamente ✅\n`
    txt += `│\n`
    txt += `╰➺ ✧ *Comando anterior:* .${oldFileName.replace('.js', '')}\n`
    txt += `╰➺ ✧ *Nuevo comando:* .${newFileName.replace('.js', '')}\n`
