let handler = async (m, { conn, args, participants, isAdmin, isBotAdmin, isOwner, isPrems, usedPrefix, command }) => {
  ...
  try {
    ...
    let txt = `╭─「 ✦ 𓆩🫏𓆪 ᴛᴏᴘ ʙᴜʀʀᴏs ᴅᴇʟ ɢʀᴜᴘᴏ ✦ 」─╮\n`
    txt += `│\n`
    ...
    console.error('Error en top burros:', e)
    return conn.sendMessage(m.chat, {
      text: '《✧》Ocurrió un error al generar el top de burros del grupo.',
      contextInfo: {
        ...rcanal.contextInfo
      }
    }, { quoted: m })
...
handler.help = ['#topburros']
handler.tags = ['fun', 'grupos']
handler.command = ['topburros', 'topburro', 'burros', 'burrotop']
handler.group = true
